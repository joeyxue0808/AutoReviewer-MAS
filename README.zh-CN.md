...
| Agent | 角色 | 职责 |
|:---:|:---|:---|
| 🏗️ | **Reviewer** (高级架构师) | 分析代码缺陷，按语言特性深度审查 |
| 🔧 | **Fixer** (重构工程师) | 生成精确的 Search/Replace 修复块 |
| 🧪 | **Tester** (测试工程师) | 在 Docker 沙盒中验证修复是否通过 |
| 🛡️ | **Critic** (红蓝对抗) | 快速挑刺，拦截低质量修复，节省沙盒开销 |
...
🔍 发现 2 个问题 (重试 0 次)
├── 1. [CRITICAL] src/auth.py:42
│   └── Token 过期未刷新
│   └── 💡 增加 refresh_token 逻辑
└── 2. [WARNING] src/models.py:15
    └── 缺少类型注解
    └── 💡 添加 type hints
```
...
A: Webhook 立即返回 `{"status": "processing"}`，审查在后台异步执行。通常 1-3 分钟完成，取决于 MR 大小和 LLM 响应速度。
